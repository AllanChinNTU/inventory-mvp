// ui.js - DOM render + events binding
import { loadDb, addWarehouse, addProduct, postTransaction, exportDbJson, importDbJson, resetDb, findProductByBarcode } from "./db.js";
...
function setMsg(id, msg, ok=true){
  $(id).innerHTML = ok ? `<span class="okText">${msg}</span>` : `<span class="dangerText">${msg}</span>`;
}
...
export function applyRoleToUI(actor){
  ...
  // tx
  ["txType","txWh","txProd","txScan","txLot","txExp","txQty","txNote","commitTxBtn"].forEach(id=>{
    $(id).disabled = txDisabled;
  });
...
export function renderAll(){
  ...
  $("prodList").innerHTML = db.products.length
    ? db.products.map(p=>`<span class="pill">${p.name}${p.barcode?`｜${p.barcode}`:""}</span>`).join(" ")
    : `<span class="muted">尚無商品</span>`;

  // selects
  $("txWh").innerHTML = db.warehouses.map(w=>`<option value="${w.id}">${w.name}</option>`).join("");
  $("txProd").innerHTML = db.products.map(p=>`<option value="${p.id}">${p.name}</option>`).join("");

  // inventory balance view
  const q = ($("search").value || "").trim().toLowerCase();
  const warnDays = parseInt($("expWarnDays").value || "30", 10);

  const rows = db.stockBalances.map(b=>{
    ...
    if(expDays !== null){
      ...
      else if(expDays <= warnDays) status = `<span class="dangerText">快到期 (${expDays}天)</span>`;
      else status = `<span class="okText">正常 (${expDays}天)</span>`;
    }

    const hay = [wh?.name, prod?.name, prod?.barcode, lot?.lotNo, lot?.expDate].filter(Boolean).join(" ").toLowerCase();
  ...
  `).join("") : `<tr><td colspan="7" class="muted">沒有符合條件的庫存資料</td></tr>`;

  // ledger view
  const tx = db.stockTransactions.slice().sort((a,b)=>b.ts.localeCompare(a.ts)).slice(0,50);
  $("txBody").innerHTML = tx.length ? tx.map(t=>{
    ...
    `;
  }).join("") : `<tr><td colspan="9" class="muted">尚無異動紀錄</td></tr>`;
}
